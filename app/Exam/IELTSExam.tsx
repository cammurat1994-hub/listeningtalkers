  ...
  return (
    <div className="flex flex-col gap-4">
      {qs.map((q, i) => {
        ...
        return (
          <div key={i} className="rounded-2xl border border-[#c8d5e8] bg-white p-5">
              ...
              {(["A","B","C","D","E"] as const).filter(l => q.options[l]).map(letter => {
                ...
                return (
                  <button key={letter} disabled={locked} onClick={() => onAnswer(key, letter)}
                    className={`rounded-2xl border p-3 text-left text-sm transition ${showCorrect ? "border-green-400 bg-green-50" : showWrong ? "border-red-400 bg-red-50" : isSelected ? "border-[#1e2d4a] bg-[#dbe4f0]" : "border-[#c8d5e8] bg-white hover:bg-[#dbe4f0]"}`}>
            ...
            {locked && q.explanation && (
              <div className="mt-3 rounded-2xl border border-[#c8d5e8] bg-[#f0f2f5] p-3">
                <p className="text-xs font-bold text-[#4a5568]">💡 {q.explanation}</p>
              </div>
            )}
          </div>
        );
...
function NoteFormRenderer({ group, sectionNum, answers, onAnswer, locked }: {
  ...
  return (
    <div className="rounded-2xl border border-[#c8d5e8] bg-white p-5">
      {data?.title && <p className="font-bold mb-4 text-center border-b border-[#c8d5e8] pb-3">{data.title}</p>}
          ...
          return (
            <div key={i} className="flex items-center gap-2 flex-wrap text-sm">
              ...
              <input type="text" value={userAns} onChange={e => onAnswer(key, e.target.value)} disabled={locked}
                className={`rounded-xl border px-3 py-1.5 text-sm w-32 text-center font-semibold ${locked ? (isCorrect ? "border-green-400 bg-green-50 text-green-700" : "border-red-400 bg-red-50 text-red-700") : "border-[#1e2d4a] bg-white"}`} />
  ...
  return (
    <div className="flex flex-col gap-3">
      {items.map((item, i) => {
        ...
        return (
          <div key={i} className="rounded-2xl border border-[#c8d5e8] bg-white p-4 text-sm flex items-center gap-1 flex-wrap">
            <span className="font-semibold text-[#4a5568] mr-1">{i + 1}.</span>
            <span>{parts[0]}</span>
            <input type="text" value={userAns} onChange={e => onAnswer(key, e.target.value)} disabled={locked}
              className={`rounded-xl border px-2 py-1 text-sm w-28 text-center font-semibold ${locked ? (isCorrect ? "border-green-400 bg-green-50 text-green-700" : "border-red-400 bg-red-50 text-red-700") : "border-[#1e2d4a] bg-white"}`} />
            {parts[1] && <span>{parts[1]}</span>}
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center w-full">
              <div className="w-full rounded-2xl border-2 border-[#1e2d4a] bg-[#f0f2f5] px-4 py-3 text-sm text-center">
                {parts.length > 1 ? (
                  <span className="flex items-center justify-center gap-1 flex-wrap">
                    <span>{parts[0]}</span>
                    <input type="text" value={userAns} onChange={e => onAnswer(key, e.target.value)} disabled={locked}
                      className={`rounded-xl border px-2 py-0.5 text-sm w-28 text-center font-semibold ${locked ? (isCorrect ? "border-green-400 bg-green-50 text-green-700" : "border-red-400 bg-red-50 text-red-700") : "border-[#1e2d4a] bg-white"}`} />
                    {parts[1] && <span>{parts[1]}</span>}
                    {locked && !isCorrect && <span className="text-xs text-green-600 font-semibold">✓ {step.answer.split("|")[0]}</span>}
                  </span>
                ) : <span>{step.text}</span>}
              </div>
              {i < steps.length - 1 && <div className="text-[#1e2d4a] text-xl font-bold leading-none py-1">↓</div>}
            </div>
          );
        })}
      </div>
      ...
      {qs.map((q, i) => {
        ...
        return (
          <div key={i} className="rounded-2xl border border-[#c8d5e8] bg-white p-4">
            ...
            <input type="text" value={userAns} onChange={e => onAnswer(key, e.target.value)} disabled={locked}
              placeholder="Your answer..." className={`w-full rounded-2xl border p-3 text-sm ${locked ? (isCorrect ? "border-green-400 bg-green-50" : "border-red-400 bg-red-50") : "border-[#c8d5e8] bg-white"}`} />
...
function MapRenderer({ group, sectionNum, answers, onAnswer, locked }: {
  ...
  function getPointColor(point: { id: number; answer: string }) {
    ...
    const userAns = answers[key];
    if (!locked) return userAns ? "bg-blue-500" : selectedPoint === point.id ? "bg-blue-700" : "bg-[#1e2d4a]";
  ...
  return (
    <div className="rounded-2xl border border-[#c8d5e8] bg-white p-4">
      {data?.imageUrl && (
        <div className="relative w-full overflow-hidden rounded-2xl border border-[#c8d5e8]" style={{ paddingBottom: "60%" }}>
      ...
      {!locked && (
        <div className="mt-4">
          <p className="text-xs font-semibold text-[#4a5568] mb-2">
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            {options.map(opt => {
              ...
              return (
                ...
                }} className={`rounded-2xl border p-2 text-xs text-left transition ${selectedPoint ? "border-[#c8d5e8] bg-white hover:border-[#1e2d4a] hover:bg-[#dbe4f0]" : "border-[#c8d5e8] bg-white opacity-50"} ${isUsed ? "opacity-40" : ""}`}>
...
function TableRenderer({ group, sectionNum, answers, onAnswer, locked }: {
  ...
  let globalAnsIdx = 0;
  return (
    <div className="rounded-2xl border border-[#c8d5e8] bg-white p-4 overflow-x-auto">
      {data?.title && <p className="font-bold mb-3 text-center">{data.title}</p>}
      <table className="w-full text-sm border-collapse">
        {headers.length > 0 && (
          <thead>
            <tr>{headers.map((h, i) => <th key={i} className="border border-[#c8d5e8] bg-[#f0f2f5] px-3 py-2 text-left font-semibold">{h}</th>)}</tr>
          </thead>
        )}
        <tbody>
          {rows.map((row, ri) => (
                  ...
                  <td key={ci} className="border border-[#c8d5e8] px-3 py-2">
                    {isBlank ? (
                      <div className="flex items-center gap-1">
                        <input type="text" value={userAns} onChange={e => onAnswer(key, e.target.value)} disabled={locked}
                          className={`rounded-xl border px-2 py-1 text-xs w-24 text-center font-semibold ${locked ? (isCorrect ? "border-green-400 bg-green-50 text-green-700" : "border-red-400 bg-red-50 text-red-700") : "border-[#1e2d4a] bg-white"}`} />
...
function MatchingRenderer({ group, sectionNum, answers, onAnswer, locked }: {
  ...
  if (data?.items && data.items.length) {
    ...
    return (
      <div className="flex flex-col gap-3">
        {options.length > 0 && (
          <div className="rounded-2xl border border-[#c8d5e8] bg-[#f0f2f5] p-3">
        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={i} className={`flex items-center gap-3 rounded-2xl border p-3 ${locked ? (isCorrect ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50") : "border-[#c8d5e8] bg-white"}`}>
              ...
              <select value={userAns} onChange={e => onAnswer(key, e.target.value)} disabled={locked}
                className={`shrink-0 rounded-xl border px-3 py-1.5 text-sm font-semibold ${locked ? (isCorrect ? "border-green-400 bg-green-50 text-green-700" : "border-red-400 bg-red-50 text-red-700") : "border-[#1e2d4a] bg-white"}`}>
                <option value="">—</option>
      ...
      {pairs.map((pair, i) => {
        ...
        return (
          <div key={i} className={`flex items-center gap-3 rounded-2xl border p-3 ${locked ? (isCorrect ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50") : "border-[#c8d5e8] bg-white"}`}>
            ...
            <span className="text-[#4a5568]">→</span>
            <input type="text" value={userAns} onChange={e => onAnswer(key, e.target.value)} disabled={locked}
              placeholder="Answer..." className={`flex-1 rounded-xl border px-3 py-1.5 text-sm ${locked ? (isCorrect ? "border-green-400 bg-green-50" : "border-red-400 bg-red-50") : "border-[#c8d5e8] bg-white"}`} />
...
function QuestionGroupView({ group, sectionNum, answers, onAnswer, locked }: {
  ...
  return (
    <div className="rounded-4xl border border-[#c8d5e8] bg-[#ffffff] p-5 shadow-sm">
      <p className="text-xs font-bold uppercase tracking-wide text-[#4a5568] mb-4">{group.label}</p>
...
const TONE: Record<string, { border: string; bg: string; title: string; desc: string; bar: string; barBg: string; pill: string }> = {
  brown:  { border: "border-[#c8d5e8]", bg: "bg-[#ffffff]", title: "text-[#1e2d4a]", desc: "text-[#4a5568]", bar: "bg-[#1e2d4a]", barBg: "bg-[#dbe4f0]", pill: "bg-[#dbe4f0] text-[#1e2d4a]" },
  ...
  purple: { border: "border-[#c8d5e8]", bg: "bg-[#ffffff]", title: "text-[#1e2d4a]", desc: "text-[#4a5568]", bar: "bg-[#1e2d4a]", barBg: "bg-[#dbe4f0]", pill: "bg-blue-100 text-blue-600" },
  ...
  return (
    <main className="min-h-screen bg-[#f0f2f5] text-[#1e2d4a]">
      <div className="mx-auto max-w-3xl px-6 py-5 flex items-center justify-between">
        <button onClick={onBack} className="rounded-2xl border border-[#c8d5e8] bg-white px-4 py-2 text-sm font-semibold hover:bg-[#dbe4f0]">← Back</button>
        <div className="text-right">
          <p className="font-bold">{title}</p>
          <p className="text-sm text-[#4a5568]">{examType}</p>
        </div>
      </div>
      <audio ref={audioRef} onEnded={handleAudioEnded} onError={handleAudioEnded} />

      {/* Top bar */}
      <div className="sticky top-0 z-50 border-b border-[#c8d5e8] bg-white shadow-sm">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-6 py-3">
            ...
            <div className="flex gap-1">
              {sections.map((_, i) => (
                <div key={i} className={`h-2 w-8 rounded-full transition-all ${
                  ...
                  i === currentSectionIndex ? "bg-[#1e2d4a]" : "bg-[#c8d5e8]"
                }`} />
              ))}
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-[#4a5568]">{formatTime(totalElapsed)}</span>
          </div>
        </div>
      </div>
        ...
        {info && (
          <div>
            <div className={`mb-6 rounded-4xl border ${tone.border} ${tone.bg} p-5`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  {activeUi === "listening" ? (
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#1e2d4a] animate-pulse">
                ...
                <button onClick={handleSkip} className="shrink-0 rounded-2xl border border-[#c8d5e8] bg-white px-4 py-2 text-xs font-bold hover:bg-[#dbe4f0]">
                  Skip →
                </button>
              </div>
            ...
            {showQuestions && currentSection && (
              <div className="flex flex-col gap-4">
                {questionsReadonly && (
                  <p className="text-xs font-semibold text-[#4a5568]">👀 Read the questions now — you&apos;ll be able to answer once the audio starts.</p>
                )}
          ...
          <div>
            ...
            {sections.map((section, si) => (
              <div key={si} className="mb-8">
                <h2 className="text-xl font-bold mb-4 border-b border-[#c8d5e8] pb-2">Section {section.number}</h2>
                <div className="flex flex-col gap-4">
            ...
            <button onClick={onFinish} className="mt-6 w-full rounded-2xl bg-[#1e2d4a] px-6 py-4 font-bold text-white hover:bg-[#162038]">
              Submit Test →
            </button>
          </div>
        )}
