...
function DictationQuestionView({ question, answer, feedback, revealed, playsUsed, maxPlays, isPlaying, onChange, onPlay, onCheck, onReveal, onRetry }: {
  ...
  return (
    <div className="rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm">
      ...
      {playsUsed > 0 && (
        <>
          <textarea value={answer} onChange={(e) => onChange(e.target.value)} disabled={feedback !== null} placeholder="Type exactly what you heard..."
...
function MapView({ question, userAnswers, checked, playsUsed, maxPlays, isPlaying, onAnswer, onCheck, onPlay }: {
  ...
  maxPlays: number;
  isPlaying: boolean;
  onAnswer: (pointId: number, optionKey: string) => void;
  onCheck: () => void;
  onPlay: () => void;
}) {
  ...
  return (
    <div className="rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-lg">🗺️ Map Labelling</h3>
        <span className="text-sm text-[#7a6258]">{playsUsed}/{maxPlays} plays</span>
      </div>
      <button onClick={onPlay} disabled={!canPlay || isPlaying}
        className={`mb-5 flex w-full items-center justify-center gap-3 rounded-2xl px-6 py-4 font-bold transition ${canPlay && !isPlaying ? "bg-[#3b2f2f] text-white hover:bg-[#2f2424]" : "cursor-not-allowed bg-[#e0c7bb] text-[#7a6258]"}`}>
        {isPlaying ? "🔊 Playing..." : canPlay ? `▶ Play${playsUsed > 0 ? " Again" : " Audio"}` : "No plays left"}
      </button>
      {playsUsed > 0 && (
        <>
          <p className="mb-3 text-sm text-[#7a6258]">
            {!checked
              ? selectedPoint ? `Point ${selectedPoint} selected — choose an option below` : "Tap a numbered point on the map, then select an answer"
              : `${correctCount}/${question.points.length} correct`}
          </p>
          <div className="relative w-full overflow-hidden rounded-[2rem] border-2 border-[#e0c7bb] bg-white" style={{ paddingBottom: "65%" }}>
            ...
            {question.points.map((point) => (
              <button key={point.id} disabled={checked}
          ...
          {!checked && (
            <div className="mt-4">
              <p className="mb-2 text-xs font-bold uppercase tracking-wide text-[#7a6258]">
                Options — {selectedPoint ? `Answering Point ${selectedPoint}` : "Select a point first"}
              </p>
              <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                {question.options.map((opt) => {
                  ...
                  return (
                    <button key={opt.key} disabled={!selectedPoint}
                      onClick={() => {
                        if (selectedPoint !== null) { onAnswer(selectedPoint, opt.key); setSelectedPoint(null); }
                      }}
                      className={`rounded-2xl border p-3 text-left text-sm transition ${
                        isSelectedForThisPoint ? "border-blue-400 bg-blue-50 font-bold" :
                        isAssigned ? "border-[#c9a99a] bg-[#f7eee8] opacity-60" :
                        selectedPoint ? "border-[#e0c7bb] bg-white hover:border-[#3b2f2f] hover:bg-[#ead7cc]" :
                        "border-[#e0c7bb] bg-white opacity-50 cursor-not-allowed"
                      }`}>
              ...
              <button onClick={onCheck} disabled={Object.keys(userAnswers).length < question.points.length}
                className="mt-4 w-full rounded-2xl bg-[#3b2f2f] px-6 py-3 font-semibold text-white disabled:opacity-40">
                {Object.keys(userAnswers).length < question.points.length
                  ? `Answer all points (${Object.keys(userAnswers).length}/${question.points.length})`
                  : "Check Answers"}
              </button>
            </div>
          )}
            ...
            <div className="mt-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className={`text-sm font-semibold ${isCorrect ? "text-green-700" : "text-red-700"}`}>
                          {isCorrect
                            ...
                            : `✗ You: ${userOption?.key || "?"}) ${userOption?.label || "No answer"} → Correct: ${correctOption?.key}) ${correctOption?.label}`}
                        </p>
                      </div>
                    </div>
                  );
              ...
              {question.points.some(p => p.explanation) && (
                <button onClick={() => setShowExplanations(!showExplanations)}
                  className="mt-4 w-full rounded-2xl border border-[#e0c7bb] bg-white px-5 py-3 text-sm font-semibold hover:bg-[#f1ded5]">
                    ...
                    return (
                      <div key={point.id} className="rounded-2xl border border-[#e0c7bb] bg-white p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#3b2f2f] text-xs font-bold text-white">{point.id}</div>
                          <p className="text-sm font-bold">{opt?.key}) {opt?.label}</p>
                        </div>
  ...
  return (
    <div className="mt-8 rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm">
      ...
      <div className="mt-5">
        ...
        <button onClick={submitComment} disabled={submitting || !newComment.trim()} className="mt-2 rounded-2xl bg-[#3b2f2f] px-5 py-2 text-sm font-semibold text-white disabled:opacity-40">
              ...
              <div key={comment.id} className="rounded-2xl border border-[#e0c7bb] bg-white p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <button onClick={() => setReplyTo(replyTo === comment.id ? null : comment.id)} className="mt-2 text-xs font-semibold text-[#7a6258] hover:text-[#3b2f2f]">
                ...
                {replyTo === comment.id && (
                  <div className="mt-3 ml-11">
                    ...
                    <button onClick={() => submitReply(comment.id)} disabled={submitting || !replyText.trim()} className="mt-2 rounded-2xl bg-[#3b2f2f] px-4 py-2 text-xs font-semibold text-white disabled:opacity-40">
...
const [ieltsPartIndex, setIeltsPartIndex] = useState(0);
    ...
    async function fetchEpisode() {
      ...
      if (!error && data) {
        ...
        const { data: allEpisodes } = await supabase.from("episodes")
          ...
          .eq("level", data.level).eq("episode_type", data.episode_type)
  ...
  function calculateScore() {
    ...
    episode.questions.forEach((q, i) => {
      ...
      } else if (type === "ielts-section") {
        const parts = episode.questions as unknown as IELTSSectionPart[];
        parts?.forEach(part => {
          part.groups?.forEach(group => {
            if (group.type === "note-completion" || group.type === "form-completion" || group.type === "sentence-completion") {
              (group.data?.items || group.data?.fields || []).forEach((_: any, bi: number) => {
                total++;
  ...
  async function saveResult() {
    ...
    await supabase.from("user_results").insert([{
      ...
      level: episode.level, episode_type: episode.episode_type, score: correct, total_questions: total
    }]);
    setResultSaved(true);
  }
      ...
      <section className="mx-auto max-w-3xl px-6 py-12">
          ...
          <div className="mt-8">
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex gap-1">
                {questions.map((_, i) => (
                  <div key={i} className={`h-2 w-2 rounded-full ${i === currentQuestion ? "bg-[#3b2f2f]" : mcqConfirmed[i] ? "bg-[#c9a99a]" : "bg-[#e0c7bb]"}`} />
                ))}
              </div>
            </div>
            <MCQQuestionView
              ...
              onToggle={(letter) => {
                ...
                if (correctArr.length === 1) {
                  ...
                  next = current.includes(letter)
                    ...
                    : current.length < correctArr.length
                      ? [...current, letter].sort()
                      : current;
                }
          ...
          <div className="mt-8">
            ...
            {testStarted && (
              <>
                ...
                <button onClick={async () => { setShowResults(true); await saveResult(); }}
                  className="mt-8 w-full rounded-2xl bg-[#3b2f2f] px-6 py-4 font-semibold text-white">Finish ✓</button>
              </>
            )}
          </div>
        )}
        ...
        {isDictation && testStarted && !showResults && question && (
          <div className="mt-8">
            ...
            />
            <div className="mt-6 flex justify-between gap-4">
              <button disabled={currentQuestion === 0} onClick={() => { setCurrentQuestion(currentQuestion - 1); setDictationPlaying(false); if (dictationAudioRef.current) dictationAudioRef.current.pause(); }}
                className="rounded-2xl border border-[#e0c7bb] bg-white px-6 py-3 font-semibold disabled:opacity-30">Previous</button>
              {currentQuestion < questions.length - 1 ? (
        ...
        {isMatching && testStarted && !showResults && (
          <div className="mt-8 flex flex-col gap-6">
            ...
            <button onClick={async () => { setShowResults(true); await saveResult(); }}
              className="w-full rounded-2xl bg-[#3b2f2f] px-6 py-4 font-semibold text-white">Finish ✓</button>
          </div>
        )}
        ...
        {isMap && !testStarted && (
          <div className="mt-8 rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm text-center">
            ...
            <p className="mt-2 text-xs text-[#7a6258]">💡 Tip: Study the map before playing the audio. Take mental notes as you listen.</p>
          ...
          <div className="mt-8">
            ...
            <MapView
              ...
              onPlay={() => {
                ...
                if (!audio || mapPlays >= 2) return;
                setMapPlays(prev => prev + 1);
                audio.currentTime = 0; audio.play(); setMapPlaying(true);
              }}
            />
            {mapChecked && (
              <button onClick={async () => { setShowResults(true); await saveResult(); }}
                className="mt-6 w-full rounded-2xl bg-[#3b2f2f] px-6 py-4 font-semibold text-white">Finish ✓</button>
            )}
          </div>
        )}
{/* ─── IELTS Section ─── */}
        {isIELTSSection && !testStarted && (
          <div className="mt-8 rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm text-center">
            ...
            <p className="mt-2 text-sm text-[#7a6258]">
              Listen to the audio carefully. You will answer questions as you listen.
            </p>
            <p className="mt-2 text-xs text-[#7a6258]">
              💡 Read the questions before pressing play.
            </p>
            <button onClick={() => setTestStarted(true)}
              className="mt-6 rounded-2xl bg-[#3b2f2f] px-8 py-4 font-semibold text-white">
              Start Practice
            </button>
          </div>
        )}
        ...
        {isIELTSSection && testStarted && !showResults && (() => {
          ...
          function nextGroup() {
            if (!isLastGroup) {
              setIeltsGroupIndex(i => i + 1);
            } else if (!isLastPart) {
              setIeltsPartIndex(i => i + 1);
              setIeltsGroupIndex(0);
              setIeltsAudioPlaying(false);
              if (ieltsAudioRef.current) ieltsAudioRef.current.pause();
            } else {
              setShowResults(true);
              saveResult();
            }
            ...
            <div className="mt-8">
              ...
              <div className="mb-4 flex items-center justify-between">
                <p className="text-sm font-semibold text-[#7a6258]">
                  Part {ieltsPartIndex + 1} of {parts.length} — Group {ieltsGroupIndex + 1} of {part.groups?.length || 1}
                </p>
                {!isSection4 && (
                  <span className={`rounded-full px-3 py-1 text-xs font-bold ${ieltsPartIndex === 0 ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"}`}>
                    {ieltsPartIndex === 0 ? "Part 1" : "Part 2"}
                  </span>
                )}
              </div>

              {/* Audio */}
              <audio ref={ieltsAudioRef} src={part.audioUrl}
                onEnded={() => setIeltsAudioPlaying(false)} />
              <button
                onClick={() => {
                  const audio = ieltsAudioRef.current;
                  if (!audio) return;
                  if (ieltsAudioPlaying) { audio.pause(); setIeltsAudioPlaying(false); }
                  else { audio.play(); setIeltsAudioPlaying(true); }
                }}
              ...
              {/* Word limit */}
              {group?.wordLimit && (
                <div className="mb-4 rounded-2xl bg-[#ead7cc] px-4 py-2 text-sm font-semibold text-[#3b2f2f]">
                  ✏️ Write {group.wordLimit}
                </div>
              )}

              {/* Group label */}
              <p className="mb-3 text-sm font-bold text-[#7a6258] uppercase tracking-wide">
                {group?.label}
              </p>

              {/* Render group by type */}
                        ...
                        return (
                          <div key={bi} className="flex items-center gap-3">
                            ...
                            <input type="text" value={userAns}
                              onChange={e => setIeltsAnswers(prev => ({ ...prev, [key]: e.target.value }))}
                              disabled={!!checked}
              ...
              {group?.type === "form-completion" && (() => {
                const fields = group.data?.fields || [];
                return (
                  <div className="rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm">
                    {group.data?.title && <p className="font-bold mb-4">{group.data.title}</p>}
                    <div className="flex flex-col gap-3">
                      {fields.map((field: any, bi: number) => {
                        const key = `${part.part}-${group.id}-${bi}`;
                        const checked = ieltsChecked[key];
                        const userAns = ieltsAnswers[key] || "";
                        const isCorrect = checked && checkAnswer(userAns, field.answer);
                        return (
                          <div key={bi} className="flex items-center gap-3">
                            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#3b2f2f] text-xs font-bold text-white">{bi + 1}</span>
                            <span className="text-sm flex-1">{field.label.replace("___", "")}</span>
                            <input type="text" value={userAns}
                              onChange={e => setIeltsAnswers(prev => ({ ...prev, [key]: e.target.value }))}
                              disabled={!!checked}
                              className={`w-32 rounded-xl border px-2 py-1 text-sm text-center font-semibold ${checked ? (isCorrect ? "border-green-400 bg-green-50 text-green-700" : "border-red-400 bg-red-50 text-red-700") : "border-[#3b2f2f] bg-white"}`} />
                        ...
                        onToggle={(letter) => {
                          ...
                          if (correctArr.length === 1) {
                            ...
                            next = current.includes(letter)
                              ...
                              : current.length < correctArr.length ? [...current, letter].sort() : current;
                          }
              ...
              {group?.type === "matching" && (
                <MatchingView
                  ...
                  userAnswers={ieltsAnswers[`${part.part}-${group.id}`] || {}}
                  checked={ieltsChecked[`${part.part}-${group.id}`] || false}
                  onAnswer={(itemIndex, optionKey) => setIeltsAnswers(prev => ({
                    ...prev,
                    [`${part.part}-${group.id}-${itemIndex}`]: optionKey,
                    [`${part.part}-${group.id}`]: { ...(prev[`${part.part}-${group.id}`] || {}), [itemIndex]: optionKey }
                  }))}
                  onCheck={() => setIeltsChecked(prev => ({ ...prev, [`${part.part}-${group.id}`]: true }))}
                />
              )}
              ...
              {group?.type === "map" && (
                <MapView
                  ...
                  userAnswers={Object.fromEntries(
                    Object.entries(ieltsAnswers).filter(([k]) => k.startsWith(`${part.part}-${group.id}-`))
                      .map(([k, v]) => [Number(k.split("-").pop()), v as string])
                  )}
                  checked={ieltsChecked[`${part.part}-${group.id}`] || false}
                  playsUsed={0}
                  maxPlays={99}
                  ...
                  <button
                    onClick={() => {
                      ...
                      items.forEach((_: any, bi: number) => {
                        newChecked[`${part.part}-${group.id}-${bi}`] = true;
                      });
                      setIeltsChecked(newChecked);
                    }}
                    className="flex-1 rounded-2xl border border-[#e0c7bb] bg-white px-4 py-3 font-semibold text-sm">
                    Check Answers
                  </button>
                ) : null}
                <button onClick={nextGroup}
                  className="flex-1 rounded-2xl bg-[#3b2f2f] px-4 py-3 font-semibold text-white text-sm">
        ...
        <div className="mt-8">
          ...
          {showComments && !userEmail && (
            <div className="mt-4 rounded-2xl border border-[#e0c7bb] bg-white p-4 text-center text-sm text-[#7a6258]">Please log in to view and post comments.</div>
          )}
        </div>
