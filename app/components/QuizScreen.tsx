...
function MapView({ question, userAnswers, checked, playsUsed, maxPlays, isPlaying, onAnswer, onCheck, onPlay }: {
  ...
  const getPointColor = (point: MapPoint) => {
    ...
    return userAnswers[point.id] === point.answer
        ...
        <>
          ...
          {!checked && (
            <div className="mt-4">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (selectedPoint !== null) {
                          onAnswer(selectedPoint, opt.key);
              ...
              <button
                onClick={onCheck}
                disabled={Object.keys(userAnswers).length < question.points.length}
  ...
  return (
    <main className="min-h-screen bg-[#f7eee8] text-[#3b2f2f]">
      <section className="mx-auto max-w-3xl px-6 py-12">
        ...
        {isMCQ && !testStarted && (
          <div className="mt-8">
            <audio ref={audioRef} src={episode.audio_url} onTimeUpdate={() => { const a = audioRef.current; if (a) setAudioProgress((a.currentTime / a.duration) * 100); }} onLoadedMetadata={() => { const a = audioRef.current; if (a) setAudioDuration(a.duration); }} onEnded={() => setIsPlaying(false)} />
        ...
        {isFill && !showResults && (
          <div className="mt-8">
            <audio ref={audioRef} src={episode.audio_url} onTimeUpdate={() => { const a = audioRef.current; if (a) setAudioProgress((a.currentTime / a.duration) * 100); }} onLoadedMetadata={() => { const a = audioRef.current; if (a) setAudioDuration(a.duration); }} onEnded={() => setIsPlaying(false)} />
            <AudioPlayer isPlaying={isPlaying} progress={audioProgress} duration={audioDuration} audioRef={audioRef} onToggle={() => { if (!testStarted) setTestStarted(true); toggleMainAudio(); }} title={episode.title} level={episode.level} />
            {testStarted && (
              <>
                <div className="mt-6 flex flex-col gap-6">
                  {questions.map((q, i) => (
                    <FillQuestionView key={i} question={q as FillQuestion} answers={fillAnswers[i] || {}} feedback={fillFeedback[i] || {}}
                      onCheck={(ans) => { const fb: Record<number, boolean> = {}; (q as FillQuestion).blanks.forEach((b, bi) => { fb[bi] = checkAnswer(ans[bi] || "", b.answer); }); setFillAnswers({ ...fillAnswers, [i]: ans }); setFillFeedback({ ...fillFeedback, [i]: fb }); }}
        ...
        {isDictation && testStarted && !showResults && question && (
          <div className="mt-8">
            ...
            <DictationQuestionView question={question as DictationQuestion} answer={dictationAnswers[currentQuestion] || ""} feedback={dictationFeedback[currentQuestion] ?? null} revealed={dictationRevealed[currentQuestion] || false} playsUsed={dictationPlays[currentQuestion] || 0} maxPlays={2} isPlaying={dictationPlaying}
              onChange={(v) => setDictationAnswers({ ...dictationAnswers, [currentQuestion]: v })}
              onPlay={() => { const audio = dictationAudioRef.current; if (!audio) return; const plays = dictationPlays[currentQuestion] || 0; if (plays >= 2) return; setDictationPlays({ ...dictationPlays, [currentQuestion]: plays + 1 }); audio.currentTime = 0; audio.play(); setDictationPlaying(true); }}
        ...
        {isShort && testStarted && !showResults && question && (
          <div className="mt-8">
            ...
            <ShortAnswerView question={question as ShortAnswerQuestion} answer={shortAnswers[currentQuestion] || ""} feedback={shortFeedback[currentQuestion] ?? null} revealed={shortRevealed[currentQuestion] || false}
        ...
        {isMap && testStarted && !showResults && questions[0] && (
          <div className="mt-8">
            ...
            />
            {mapChecked && (
              <button onClick={async () => { setShowResults(true); await saveResult(); }} className="mt-6 w-full rounded-2xl bg-[#3b2f2f] px-6 py-4 font-semibold text-white">Finish ✓</button>
            )}
          </div>
        )}
