...
export default function AdminScreen({ onBack }: Props) {
  ...
  const [editingEpisodeId, setEditingEpisodeId] =
  useState<string | null>(null);
  const [title, setTitle] = useState("");
  const [audioFile, setAudioFile] = useState<File | null>(null);
  const [uploading, setUploading] = useState(false);
const [publishedEpisodes, setPublishedEpisodes] =
  useState<PublishedEpisode[]>([]);
...
async function fetchEpisodes() {
  ...
  async function publishEpisode() {
    if (!title || !audioFile) {
      alert("Please enter episode title and upload main audio.");
      return;
    }
    ...
    try {
      const audioUrl = await uploadAudioFile(audioFile, "episode");
    ...
    <main className="min-h-screen bg-[#f7eee8] text-[#3b2f2f]">
      <section className="mx-auto max-w-5xl px-6 py-12">
        ...
        <div className="mt-10 rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm md:p-8">
          <h2 className="text-3xl font-bold">Create New Episode</h2>
        ...
        <div className="mt-8 rounded-[2rem] border border-[#e0c7bb] bg-[#fffaf7] p-6 shadow-sm md:p-8">
          ...
          <div className="mt-8 flex flex-col gap-6">
            {vocabularyItems.map((item, index) => (
              <div
                key={index}
                className="rounded-[1.5rem] border border-[#e0c7bb] bg-white p-5"
              >
                <div className="flex items-center justify-between gap-4">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-3">
                  <div>
                    ...
                    <input
                      ...
                      value={item.word}
                      onChange={(e) =>
                        updateVocabularyItem(index, "word", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={item.type}
                      onChange={(e) =>
                        updateVocabularyItem(index, "type", e.target.value)
                      }
                      placeholder="verb"
                      className="w-full rounded-2xl border border-[#e0c7bb] bg-[#fffaf7] p-4"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={item.meaning}
                      onChange={(e) =>
                        updateVocabularyItem(index, "meaning", e.target.value)
                      }
                ...
                <div className="mt-4 grid gap-3">
                  {(["A", "B", "C", "D", "E"] as const).map((letter) => (
                    <div key={letter} className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#ead7cc] font-bold">
                        {letter}
                      </div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateOption(index, letter, e.target.value)
                        }
